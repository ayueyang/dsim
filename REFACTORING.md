...
活口径以 `AGENTS.md` §5 文件树 + `scripts/check-agents-tree.sh` 为准，历史数字仅存档即可。
...
| W23 | P3（**可选·未批准**） | 一次性清理旧 ANDROID_ID 身份残留（本机与对端的 ghost 档案 / `DEV_<id>` REMOTE_SHADOW） | XS–S | 需用户批准 |
...
### W23（P3，可选·一次性；未批准）覆盖安装后的旧身份残留清理 — 登记，先不要写代码

**现象**（实测证据见 `FIXES_2026-09-19.md` 阶段 1 回归第 6 条与审计 F-4）：T1.3 把 deviceId 从 `ANDROID_ID` 换成应用自管 UUID 后，覆盖安装的设备上会多出一个旧身份的 LOCAL 档案（同机两条 `isLocalDevice=1`）；**对端**那边也留着旧身份的 `DEV_<旧 ANDROID_ID>_…` REMOTE_SHADOW 卡与一条永不回线的 ghost 设备行。

**给定裁决背景**：修复计划明确「不做 ANDROID_ID→UUID 迁移、项目无发布用户、旧数据可弃」，所以本工作项**始终可选**——新测试一律 `pm clear` 重装即可规避。不要在没有批准的情况下实现它。

**若被批准，最小边界**：一次性的 grep 可见的清理动作（如维护 SQL / 首启自检），只处理 `deviceId != 当前身份` 的 LOCAL 行与其 `DEV_` 前缀 mappingKey；**对端**一侧同样执行一次（两侧语义一致）。**不得**为此新增任何在线协议消息去「通知对端」——那会扩大变更面并把旧账新账混在一起。
