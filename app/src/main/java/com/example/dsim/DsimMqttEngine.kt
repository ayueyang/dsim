...
object DsimMqttEngine {
    ...
    fun connectWithPairCode(pairCode: String) {
        scope.launch {
            _connectionStatus.value = "正在连接国内云端节点..."
            // 使用用户自定义的长暗号作为专属频道
            ...
            client = MqttClient.builder()
                ...
                .automaticReconnectWithDefaultConfig() // 底层自带断网无限重连
                .buildAsync()

            client?.connect()?.whenComplete { _, throwable ->
                if (throwable != null) {
                    _connectionStatus.value = "云端连接失败: ${throwable.message}"
                } else {
                    _connectionStatus.value = "已接入云端！当前专属频道: $pairCode"
                    subscribeToTopic() // 连上后立刻监听该频道
                }
            }
        }
    }
    ...
    private fun subscribeToTopic() {
        client?.subscribeWith()
            ...
            ?.callback { publish ->
                ...
                Log.d(TAG, "云端频道收到消息: $payload")
                try {
                    val json = JSONObject(payload)
                    if (json.optString("action") == "SYNC_SMS") {
                        scope.launch {
                            _incomingSmsEvent.emit(Pair(json.getString("sender"), json.getString("body")))
                        }
                    }
    ...
    fun sendSmsToCloud(sender: String, body: String) {
        if (client == null || currentTopic.isEmpty()) {
            Log.e(TAG, "未连接云端，无法发送")
            return
        }
        ...
        client?.publishWith()
            ...
            ?.qos(MqttQos.AT_LEAST_ONCE) // QoS 1: 保证消息至少送达一次防丢失
            ?.send()
        Log.d(TAG, "短信已发射至云端频道")
    }
}